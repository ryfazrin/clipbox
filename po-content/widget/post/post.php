...
class Post implements ExtensionInterface
{
	...
	*/
     ...
     * ----BUATAN RYAN FAZRIN----
     *
	 * Fungsi ini digunakan untuk mengambil daftar post ditambah dengan pagination
	 *
	 * This function use to get list of post.
	 *
	 * $limit = integer
	 ...
	 * $lang = WEB_LANG_ID
	*/
    public function getAllPost($limit, $order, $page, $lang)
    {
	 ...
	 * ----BUATAN RYAN FAZRIN----
	 *
	 * Fungsi ini digunakan untuk membuat nomor halaman pada halaman utama atau home.
	 *
	...
	*/
    public function getAllPaging($limit, $page, $type, $prev, $next)
    {
			...
			$pagination = $this->core->popaging->navPage($page, $totalpage, BASE_URL, 'home', 'page', $type, $prev, $next);
		return $pagination;
	}
	...
	*/
	 ...
	 * $link = boolean
	*/
	public function getPostTag($tagseo, $sep = ', ', $open = NULL, $close = NULL, $link = true)
    {
		$tag = '';
		...
		foreach($tagseoexp as $exp){
			$tags = $this->core->podb->from('tag')
				...
				->fetch();
			if ($link) {
				$tag .= $open.'<a href="'.WEB_URL.'tag/'.$tags['tag_seo'].'">'.ucfirst($tags['title']).'</a>'.$close.$sep;
			} else {
				$tag .= $tags['title'].$sep;
			}
		}
